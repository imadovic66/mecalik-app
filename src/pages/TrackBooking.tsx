...
export default function TrackBooking() {
  ...
  useEffect(() => {
    if (booking?.status === 'completed' && isGuestBooking) {
      const t = setTimeout(() => setShowAccountPopup(true), 2000)
      return () => clearTimeout(t)
    }
  }, [booking?.status, isGuestBooking])
      ...
      {booking?.status === 'completed' && (
        <div style={{ width: '100%', maxWidth: '440px', marginTop: '16px', background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: '16px', overflow: 'hidden' }}>
          ...
          <div style={{ padding: '20px 24px', display: 'flex', flexDirection: 'column', gap: '12px' }}>
            ...
            <div style={{ fontSize: '13px', color: 'rgba(255,255,255,0.4)', marginBottom: '8px', lineHeight: 1.5 }}>
              {isFr
                ? "Votre avis sur Google aide d'autres Casablancais à nous trouver."
                : 'Your Google review helps other Casablanca customers find us.'}
            </div>

            {/* Decorative star row — each star opens Google */}
            <div style={{ display: 'flex', gap: '6px', justifyContent: 'center', margin: '4px 0' }}>
              {[1, 2, 3, 4, 5].map(star => (
                <a
                  ...
                  style={{ fontSize: '36px', textDecoration: 'none', cursor: 'pointer' }}
                >⭐</a>
              ))}
            </div>
            ...
            <p style={{ fontSize: '11px', color: 'rgba(255,255,255,0.25)', textAlign: 'center', margin: 0 }}>
              {isFr ? 'Cela prend moins de 30 secondes' : 'Takes less than 30 seconds'}
            </p>
          </div>
        </div>
      )}
