...
const SERVICE_TEMPLATES: Record<string, ServiceDetail[]> = {
  vidange: [
    { type: 'product', name: 'Huile Moteur', brand: 'Total', quantity: '5L', unit_price: 180 },
    { type: 'part',    name: 'Filtre à huile', brand: '', quantity: '1', unit_price: 45 },
    { type: 'part',    name: 'Joint de vidange', brand: '', quantity: '1', unit_price: 8 },
  ],
  batterie: [
    { type: 'part', name: 'Batterie', brand: '', quantity: '1', unit_price: 350 },
  ],
  pneus: [
    { type: 'part',  name: 'Pneu', brand: '', quantity: '1', unit_price: 350 },
...
function getTemplate(serviceName: string): ServiceDetail[] {
  ...
  return [{ type: 'product', name: '', brand: '', quantity: '1', unit_price: 0 }]
}
...
export default function ReservationsTab({ bookings, loading, mechanics, onSelectBooking, onRefresh }: Props) {
  ...
  const addServiceDetail = (bookingId: string, booking: Booking) => {
    ...
    setLocalDetails(prev => ({
      ...prev,
      [bookingId]: [...current, { type: 'product', name: '', brand: '', quantity: '1', unit_price: 0 }],
    }))
  }
                  ...
                  <div style={{ padding: '16px 20px', background: 'rgba(255,255,255,0.02)', borderTop: '1px solid rgba(255,255,255,0.05)' }}>
                    <div style={{ fontSize: '13px', fontWeight: 700, color: 'white', marginBottom: '14px' }}>
                      ...
                      {booking.service_details?.length ? (
                        <span style={{ marginLeft: '8px', fontSize: '11px', fontWeight: 500, color: 'rgba(255,255,255,0.3)' }}>
                          (sauvegardé)
                        </span>
                      ) : null}
                    </div>
                    ...
                    {/* Column headers */}
                    {details.length > 0 && (
                      <div style={{ display: 'grid', gridTemplateColumns: '1fr 120px 70px 70px 90px 32px', gap: '8px', marginBottom: '6px', padding: '0 2px' }}>
                        {['Nom / Produit', 'Marque', 'Qté', 'P.U. (MAD)', 'Total', ''].map(h => (
                    ...
                    {details.map((item, idx) => (
                      <div key={idx} style={{ display: 'grid', gridTemplateColumns: '1fr 120px 70px 70px 90px 32px', gap: '8px', marginBottom: '8px', alignItems: 'center' }}>
                        <input
                          value={item.name}
                          placeholder="Huile Total 5W40 / Filtre à huile…"
                          onChange={e => updateServiceDetail(booking.id, booking, idx, 'name', e.target.value)}
                          style={inputStyle}
                        />
                        <input
                          value={item.brand || ''}
                          placeholder="Total, Bosch…"
                        ...
                        <input
                          ...
                          style={inputStyle}
                        />
                        <span style={{ fontSize: '13px', color: '#43BCC9', fontWeight: 700, textAlign: 'right', paddingRight: '4px' }}>
                          {Math.round((item.unit_price || 0) * parseFloat(item.quantity || '1'))} MAD
                        </span>
                        <button
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginTop: '10px', flexWrap: 'wrap' }}>
                      ...
                      {/* Parts total */}
                      {details.length > 0 && (
                        <div style={{ marginLeft: 'auto', display: 'flex', alignItems: 'center', gap: '8px', padding: '7px 14px', borderRadius: '8px', background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.06)' }}>
                          <span style={{ fontSize: '12px', color: 'rgba(255,255,255,0.4)' }}>Total pièces :</span>
                          <span style={{ fontSize: '14px', fontWeight: 700, color: '#43BCC9' }}>{Math.round(detailsTotal)} MAD</span>
                        </div>
                      )}
                    </div>
                  </div>
